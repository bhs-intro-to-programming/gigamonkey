...
const drawSnomanNose = (x, headY, headRadius) => {
...
const drawSnomanMouth = (headRadius, headY, x) => {
  for (let i = 0; i < 5; i++) {
...
const drawSnowmanHat = (headY, headRadius, x) => {
...
const drawSnowmanButtons = (torsoY, x, torsoSize) => {
  const torsoRadius = torsoSize / 2;
  for (let i = 0; i < 3; i++) {
...
const drawSnowmanButt = (buttSize, x, buttY) => {
...
const drawPicture = (horizon, base, size) => {
  ...
  // Draw the head
  drawSnowmanHead(headY, headSize, x);

  // Draw the eyes
  drawSnowmanEyes(headY, headSize, x);

  // Draw the nose
  drawSnomanNose(x, headY, headRadius);

  // Draw the mouth
  drawSnomanMouth(headRadius, headY, x);

  // Draw the hat
  drawSnowmanHat(headY, headRadius, x);

  // Draw the torso
  drawSnowmanTorso(torsoSize, x, torsoY);

  // Draw the arms
  drawSnowmanArms(x, torsoY, torsoSize);

  // Draw the buttons
  drawSnowmanButtons(torsoY, x, torsoSize);

  // Draw the butt
  drawSnowmanButt(buttSize, x, buttY)
};
