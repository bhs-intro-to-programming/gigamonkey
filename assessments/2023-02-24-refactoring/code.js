const drawTheBackground = (width, horizon, height) => {
  drawFilledRect(0, 0, width, horizon, '#ddeeff');
  drawFilledRect(0, horizon, width, height, 'white');
  drawLine(0, horizon, width, horizon, '#bbb');
}
const makeProportions = (headP, torsoP, buttP) => {
 let x = width / 2;
...
const drawTheHead = (headY, headSize, headRadius) => {
  let headY = (base - size) + headSize / 2;
  let headSize = size * (headP / total);
  let headRadius = headSize / 2;
...
// Draw the torso
const drawTorso = (torsoY, x, torsoRadius) => {
  let torsoY = torsoSize / 2;
  drawCircle(x, torsoY, torsoRadius + 2, 'black', 3);
  drawFilledCircle(x, torsoY, torsoRadius, 'white', 3);
}
const drawTheArms = (x, torsoRadius, torsoY) => {
...
// Draw the buttons
const drawTheButtons = (x, torsoY, torsoRadius)
for (let i = 0; i < 3; i++) {
