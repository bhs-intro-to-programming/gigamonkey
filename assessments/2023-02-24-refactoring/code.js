...
const drawDots = (change, direction, size, count, start) => {
  for (let i = 0; i < count; i++) {
    drawFilledCircle(start.x + direction === 'x' ? i * change : 0, start.y + direction === 'y' ? i * change : 0, size, 'black')
  }
}

const drawNose = (length, x, headY) => {
  drawFilledTriangle(x, headY, x + length, headY + length * 0.2, x, headY + length * 0.3, 'orange');
}

const drawTop = (headSize) => {
  // Draw the head
  ...
  // Draw the eyes
  drawDots(headRadius * 0.25, 'x', 4, 2, {x: x-headRadius*0.25, y: headY-headRadius*0.25 })
...
const drawPicture = (horizon, base, size) => {
  ...
  */
  ...
  // Draw the buttons
  drawDots(torsoRadius * 0.25, torsoY + torsoRadius*0.25, )
  torsoRadius * 0.5 + torsoRadius * 0.5
  for (let i = 0; i < 3; i++) {
