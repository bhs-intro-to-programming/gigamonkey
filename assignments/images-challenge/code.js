...
const fillWithCircles = (r, color) => {
  ...
  for (let y = offsety + r; y < numy * d + r; y += d) {
    for (let x = offsetx + r; x < numx * d + r; x += d) {
  ...
  for (let r = 0; r < num; r++) {
    for (let c = 0; c < num; c++) {
      ...
      if (num === 1 || ((r % (num - 1)) * (c % (num - 1)) === 0)) {
        console.log(`drawing at ${xOffset + r * d},${yOffset + c * d}`);
...
const checkerboard = (n, scale=1) => {
  const big = Math.min(width, height) * scale;
